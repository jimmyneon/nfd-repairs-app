...
ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS reservation_transaction_id TEXT;
...
ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS reservation_refunded_at TIMESTAMPTZ;
ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS reservation_refund_amount DECIMAL(10,2);
...
COMMENT ON COLUMN enquiries.reservation_transaction_id IS 'Verified SumUp transaction id for the paid generic repair reservation.';
COMMENT ON COLUMN enquiries.reservation_payment_status IS 'Reservation checkout/payment state, e.g. PENDING, PAID, FAILED, EXPIRED, REFUNDED.';
...
COMMENT ON COLUMN enquiries.reservation_refunded_at IS 'When the generic repair reservation was successfully refunded.';
COMMENT ON COLUMN enquiries.reservation_refund_amount IS 'Amount successfully refunded for the generic repair reservation.';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_enquiries_reservation_transaction_id
ON enquiries(reservation_transaction_id)
