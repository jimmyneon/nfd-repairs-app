...
WITH settings AS (
    SELECT 
        MAX(CASE WHEN setting_key = 'google_review_enabled' THEN setting_value END) as review_enabled,
        MAX(CASE WHEN setting_key = 'google_review_link' THEN setting_value END) as review_link,
        MAX(CASE WHEN setting_key = 'auto_review_request_enabled' THEN setting_value END) as auto_enabled,
        MAX(CASE WHEN setting_key = 'review_request_delay_hours' THEN setting_value END) as delay_hours
    FROM admin_settings
    WHERE setting_key IN ('google_review_enabled', 'google_review_link', 'auto_review_request_enabled', 'review_request_delay_hours')
),
review_stats AS (
    SELECT 
        COUNT(*) as total,
...
SELECT 
    ...
    CASE 
        WHEN (SELECT setting_value FROM admin_settings WHERE setting_key = 'google_review_enabled') != 'true' 
