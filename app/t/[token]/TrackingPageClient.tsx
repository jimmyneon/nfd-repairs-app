...
export default function TrackingPageClient({ resolvedToken, shortToken }: { resolvedToken: string; shortToken: string }) {
  ...
  const [jobEvents, setJobEvents] = useState<{ created_at: string; status: string | null }[]>([])
  ...
  const loadJob = useCallback(async (showSpinner = false) => {
    ...
    try {
      const res = await fetch(`/api/tracking/${resolvedToken}`)
      if (res.status === 410) {
        setIsExpired(true)
        setLoading(false)
        return
      ...
      if (events.length > 0) {
        ...
        // Build a map of status -> timestamp from events. The API now
        // returns a parsed `status` key (not the raw staff-authored
        // message), so no client-side parsing is needed.
        const timestamps: Record<string, string> = {}
        for (const event of events) {
          if (event.status && !timestamps[event.status]) {
            timestamps[event.status] = event.created_at
          }
        }
        ...
        if (['DELAYED', 'AWAITING_CUSTOMER'].includes(jobData.status) && events.length > 1) {
          ...
          for (let i = 0; i < events.length; i++) {
            const statusKey = events[i].status
  ...
  const buildStatusSteps = () => {
    ...
    // We rely on the status field only — diagnosis_notes/diagnostic_report are
    // internal free-text fields and are no longer exposed to the customer.
    if (journeyBaseStatus === 'DIAGNOSTIC' || (statusTimestamps && statusTimestamps.DIAGNOSTIC)) {
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-2 border-gray-100 dark:border-gray-700">
          ...
          >
            ...
            {/* Customer-facing update — only shown when staff have published
                a dedicated customer update via the tracking page. Free-text
                internal notes (customer_notes, diagnosis_notes, diagnostic_report)
                are no longer exposed to the customer. */}
            ...
            <div className="px-6 pb-6 text-center border-t border-gray-100">
              ...
              <div className="flex justify-center mb-4">
                <div className="bg-white p-4 rounded-xl shadow-inner border-2 border-gray-100">
                  <QRCodeDisplay value={shortTrackingLink(shortToken)} size={200} />
                </div>
              </div>
