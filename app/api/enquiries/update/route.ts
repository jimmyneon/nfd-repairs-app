  ...
  try {
    ...
    switch (action) {
      ...
      case 'send_personalised_quote': {
        ...
        updateFields.quote_sent_method = method
        updateFields.status = 'pending'
        // Store the display price (range or single) so the customer-facing
        // quote acceptance page can show it
        updateFields.display_price = priceHigh !== null ? `£${price}-£${priceHigh}` : `£${price}`
    ...
    // Staff-initiated actions don't need an alert — the staff member just
    // performed the action, and every insert triggers a push to their device.
    if (notificationTitle && !staffOnlyActions.includes(action) && action !== 'schedule_remote_session') {
      try {
